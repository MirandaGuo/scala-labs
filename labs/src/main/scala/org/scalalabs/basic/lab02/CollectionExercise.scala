...
object CollectionExercise01 {
  ...
  def googleCodeJamGooglerese(lines: String*): Seq[String] = {
    ...
    val letters = List('y', 'h', 'e', 's', 'o', 'c', 'v', 'x', 'd', 'u', 'i', 'g', 'l',
      'b', 'k', 'r', 'z', 't', 'n', 'w', 'j', 'p', 'f', 'm', 'a', 'q')

    for {
      line <- lines
      chars = line.toCharArray
      output = chars.map(ele => {
        if(ele == ' ') ele
        else letters(ele - 'a')
      }).mkString("")
...
object CollectionExercise03 {
  ...
  def checkValuesIncrease[T <% Ordered[T]](seq: Seq[T]): Boolean = seq match {
    ...
    case x :: xs if(x < xs.head) => checkValuesIncrease(xs)
...
object CollectionExercise04 {
  ...
  def calcLengthLongestWord(lines: String*): Int = {
     val charLines: Seq[Array[String]] = for(line <- lines) yield {
      line.split(" ")
    }

    charLines.foldLeft(Seq[String]())((r, c) => {
       r ++ c
...
object CollectionExercise05 {
  /**
    ...
    */
  def filterWithFoldLeft(seq: Seq[Int]): Seq[Int] = seq.foldLeft(Seq[Int]())((r, c) => {
     if(c % 2 == 0 ) r :+ c
     else r
  })
  ...
  /**
    ...
    */
  def groupByWithFoldLeft(seq: Seq[Int]): Map[Boolean, Seq[Int]] = seq.foldLeft(Map(true-> Seq[Int](), false ->Seq[Int]()))((r, c) => {


    if(c % 2 == 0) Map[Boolean, Seq[Int]](true -> (r(true) :+ c), false -> r(false))
    else Map[Boolean, Seq[Int]](true -> r(true), false -> (r(false) :+ c))
